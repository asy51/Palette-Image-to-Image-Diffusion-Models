...
class MoonCometDataset(SliceDataset):
    def __init__(self, task='translatebone', knees=None, **kwargs):
        ...
        super().__init__(knees=knees, **kwargs)

    def __getitem__(self, ndx):
        slc = super().__getitem__(ndx)
        
        bmel = slc['BMELT'] if 'BMELT' in slc else torch.zeros((1, self.img_size, self.img_size))
        ...
        if self.task == 'translatebone':
            ...
            notbone = (slc['BONE_TSE'] == 0).to(torch.uint8)
            mask = bone
            cond = (tse * notbone * 2 - 1) + (torch.randn_like(tse).to(torch.float32) * mask)
            mask_img = (tse * notbone * 2 - 1)
            target = tse
        ...
        if self.task == 'inpaintbone':
            # ret['mask'] = mask
            ret['mask_image'] = mask_img * 2 - 1
        return ret
...
#     def __init__(self, img_size=320, **kwargs):
#         kds = KneeDataset()
#         kds.knees = [knee for knee in kds.knees if all(knee.path[k] for k in ['IMG_TSE', 'DESS2TSE', 'BMELT'])]
#         super().__init__(kds, img_size=img_size, **kwargs)
